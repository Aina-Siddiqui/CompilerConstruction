...
def wordSplitter(filename):
    seperators=['(',')',','':']
    operators=['+','-','*','/','%','**','++','__','=','+=','-=','*=','/=','%=','//=','**=','&=','|=','^=','>>=','<<=''==','!=','<','>','>=','<=','AND','OR','NOT','XOR','NOR','NAND','XNOR','&','|','^','~','<<','>>']
    ...
    with open(filename,'r') as file:
        ...
        #single line comment and multi line comment will be ignored after making
...
# combining the whole string into a single token 
combined = []
is_quoted = False
quote_type = ''
temp = ''
